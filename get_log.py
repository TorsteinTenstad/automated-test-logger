...
def get_log(logs_dir=None) -> Log:
    log = Log(None, [])
    logs_dir = logs_dir if logs_dir is not None else pathlib.Path(
        __main__.__file__).parent.joinpath(AutomatedTestLogger.DEFAULT_LOG_DIR)
...
def get_logged_property_against_time(log: Log, class_name_filter: str = None, function_name_filter: str = None, property_extraction_function: Callable[[any, any], any] = None) -> Tuple[List[datetime], List[any]]:
    ...
    filtered_log_data = [x for x in log.data if filter(
        x.class_name, x.function_name)]
    t = [datetime.strptime(x.timestamp, AutomatedTestLogger.TIME_FORMAT)
         for x in filtered_log_data]
    property = [property_extraction_function(
        x.args, x.return_val) for x in filtered_log_data]
    return t, property
